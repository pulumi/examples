...
This example deploys an AKS cluster, creates an Azure Active AD application, creates an Azure Managed Identity and sets credentials to manage access to the cluster. Using Azure Managed Identity instead of an Azure Service Princiapl reduces the complexity associated with managing an AKS cluster's identity.
