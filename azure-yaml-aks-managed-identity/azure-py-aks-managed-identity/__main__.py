...
private_key = tls.PrivateKey("privateKey",
    algorithm="RSA",
    rsa_bits=4096)
identity = azure_native.managedidentity.UserAssignedIdentity("identity", resource_group_name=resource_group.name)
...
admin_credentials = azure_native.containerservice.list_managed_cluster_admin_credentials_output(resource_group_name=resource_group.name,
...
pulumi.export("kubeconfig", admin_credentials.apply(lambda admin_credentials: base64.b64decode(admin_credentials.kubeconfigs[0].value.encode()).decode()))
