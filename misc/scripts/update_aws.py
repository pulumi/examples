...
if args.goversion:
    go_mod_files = [
        f
        for f in sp.check_output(["git", "ls-files", "**go.mod"])
        .decode("utf-8")
        .split("\n")
        if f
    ]
...
if args.nodeversion:
    pkg_json_files = [
        f
        for f in sp.check_output(["git", "ls-files", "**package.json"])
        .decode("utf-8")
        .split("\n")
        if f
    ]
...
if args.pyversion:
    ...
    for f in req_txt_files:
        ...
        if "pulumi-aws" in contents:
            with open(f, "w") as fp:
                c = re.sub(
                    r"pulumi-aws[=][=][.\w]+", f"pulumi-aws=={args.pyversion}", contents
                )
                fp.write(c)
...
if args.javaversion:
    pom_xml_files = [
        f
        for f in sp.check_output(["git", "ls-files", "**pom.xml"])
        .decode("utf-8")
        .split("\n")
        if f
    ]
...
if args.csharpversion:
    csproj_files = [
        f
        for f in sp.check_output(["git", "ls-files", "**.csproj"])
        .decode("utf-8")
        .split("\n")
        if f
    ]
