...
cluster = do.KubernetesCluster(
    ...
    node_pool=do.KubernetesClusterNodePoolArgs(
        name="default", size="s-2vcpu-2gb", node_count=node_count
    ),
)

k8s_provider = Provider(
    "do-k8s", kubeconfig=cluster.kube_configs.apply(lambda c: c[0].raw_config)
)
...
ingress = Service(
    ...
    opts=ResourceOptions(
        ...
        custom_timeouts=CustomTimeouts(create="15m", delete="15m"),
    ),
)
...
if domain_name:
    ...
    cname_record = do.DnsRecord(
        "do-domain-name", domain=domain.name, type="CNAME", name="www", value="@"
    )
