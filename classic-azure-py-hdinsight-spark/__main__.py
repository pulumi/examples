...
storage_container = storage.Container(
    "spark", storage_account_name=storage_account.name, container_access_type="private"
)
...
spark_cluster = hdinsight.SparkCluster(
    ...
    resource_group_name=resource_group.name,
    cluster_version="3.6",
    component_version=hdinsight.SparkClusterComponentVersionArgs(spark="2.3"),
    tier="Standard",
    storage_accounts=[
    ...
    gateway=hdinsight.SparkClusterGatewayArgs(username=username, password=password),
    roles=hdinsight.SparkClusterRolesArgs(
        head_node=hdinsight.SparkClusterRolesHeadNodeArgs(
            vm_size="Standard_D12_v2", username=username, password=password
        ),
...
export(
    "endpoint",
    spark_cluster.https_endpoint.apply(lambda endpoint: "https://" + endpoint),
)
