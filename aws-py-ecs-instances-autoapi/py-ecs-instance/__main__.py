...
ecs_instance_profile = aws.iam.InstanceProfile(
    "ecs-iam-instance-profile", role=ecs_instance_role.name
)
...
ecs_instance_ami = aws.ec2.get_ami(
    most_recent="true",
    owners=["amazon"],
    filters=[{"name": "name", "values": ["amzn2-ami-ecs-hvm-*-x86_64-*"]}],
)
...
wl = aws.lb.Listener(
    ...
    port=80,
    default_actions=[
        aws.lb.ListenerDefaultActionArgs(type="forward", target_group_arn=atg.arn)
    ],
)
...
pulumi.export(
    "NOTE",
    "You may have to wait a minute for AWS to spin up the service. So if the URL throws a 503 error, try again after a bit.",
)
