...
function convertExample(example: Example): SerializableExample {
    return {
        id: example.id,
        title: example.title,
...
async function run(): Promise<ExampleRunResult> {
    ...
    const stackName = "moolumi/examples-api";
    ...
    try {
        ...
        const readme = fs.readFileSync(`${workDir}/README.md`, "utf-8");
        const parsed = marked.marked.parse(readme);
        ...
        const example = {
            ...
            title: name,
            description,
            url: `https://github.com/pulumi/examples/tree/master/${exampleID}}`,
            readme,
            program: {
                name: settings.name,
                settings,
        ...
        await writeFullExample(example);
        ...
        // On failure, write whatever we can, even if that's only a README (since that's all we have today).
        const example = {
            id: exampleID,
            title: name,
            description,
        ...
        await writeMinimalExample(example);
...
async function writeFullExample(example: Example) {
    const convertedExample = convertExample(example);
    const apiURL = await getAPIURL();

    const response = await nodeFetch(`${apiURL}/examples/${example.id}`, {
        method: "PUT",
        body: JSON.stringify({ example: convertedExample }),
        headers: {
...
async function getAPIURL() {
    ...
    return stackResources.filter(resource => resource.type === "pulumi:pulumi:Stack")[0].outputs.apiURL;
}

async function writeMinimalExample(example: MinimalExample) {
    const apiURL = await getAPIURL();
...
run()
    .then(result => {
        console.log(`Run of ${result.exampleID} ${result.succeeded ? "succeeded" : "failed"}.`);
    })
    .catch(error => {
