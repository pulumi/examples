...
langserve_ecr_repository = aws.ecr.Repository(
...
langserve_ecr_life_cycle_policy = aws.ecr.LifecyclePolicy(
...
langserve_ecr_image = docker.Image(
    "langserve-ecr-image",
...
langserve_rt = aws.ec2.RouteTable(
    "langserve-rt",
...
langserve_igw = aws.ec2.InternetGateway(
    "langserve-igw",
    vpc_id=langserve_vpc.id,
    tags={
        "Name": f"{pulumi_project}-{pulumi_stack}",
...
langserve_subnet2 = aws.ec2.Subnet(
    "langserve-subnet2",
    vpc_id=langserve_vpc.id,
...
langserve_subnet1_rt_assoc = aws.ec2.RouteTableAssociation(
    "langserve-subnet1-rt-assoc",
    subnet_id=langserve_subnet1.id,
    route_table_id=langserve_rt.id,
)
langserve_subnet2_rt_assoc = aws.ec2.RouteTableAssociation(
    "langserve-subnet2-rt-assoc",
    subnet_id=langserve_subnet2.id,
    route_table_id=langserve_rt.id,
)
langserve_ecs_cluster = aws.ecs.Cluster(
...
langserve_cluster_capacity_providers = aws.ecs.ClusterCapacityProviders(
...
langserve_security_group = aws.ec2.SecurityGroup(
...
langserve_load_balancer = aws.lb.LoadBalancer(
...
langserve_target_group = aws.lb.TargetGroup(
...
langserve_listener = aws.lb.Listener(
    "langserve-listener",
...
langserve_log_group = aws.cloudwatch.LogGroup(
    "langserve-log-group", retention_in_days=7
)
langserve_key = aws.kms.Key(
    "langserve-key",
...
langserve_ssm_parameter = aws.ssm.Parameter(
...
langserve_execution_role = aws.iam.Role(
    ...
    inline_policies=[
        aws.iam.RoleInlinePolicyArgs(
            ...
            policy=pulumi.Output.all(
                langserve_ssm_parameter.arn, langserve_key.arn
            ).apply(
...
langserve_task_role = aws.iam.Role(
    "langserve-task-role",
    assume_role_policy=json.dumps(
        {
            "Statement": [
...
langserve_task_definition = aws.ecs.TaskDefinition(
...
langserve_ecs_security_group = aws.ec2.SecurityGroup(
...
langserve_service_discovery_namespace = aws.servicediscovery.PrivateDnsNamespace(
    "langserve-service-discovery-namespace",
    name=f"{pulumi_stack}.{pulumi_project}.local",
    vpc=langserve_vpc.id,
)
langserve_service = aws.ecs.Service(
    "langserve-service",
...
pulumi.export(
    "url", langserve_load_balancer.dns_name.apply(lambda dns_name: f"http://{dns_name}")
)
