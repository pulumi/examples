...
{
    var defaultVpc = Ec2.GetVpc.Invoke(new()
    {
        Default = true
    });

    // Read back the default VPC and public subnets, which we will use.
    var vpcId = defaultVpc.Apply(vpc => vpc.Id);

    var subnetIds = Ec2.GetSubnetIds.Invoke(new () { VpcId = vpcId })
        .Apply(s => s.Ids);
...
""Statement"": [{
    ...
    var webLb = new Alb.LoadBalancer("web-lb", new()
    {
        Subnets = subnetIds,
        SecurityGroups = { webSg.Id }
    });

    var webTg = new Alb.TargetGroup("web-tg", new()
    {
        Port = 80,
        Protocol = "HTTP",
        TargetType = "ip",
    ...
    var webListener = new Alb.Listener("web-listener", new()
    {
        LoadBalancerArn = webLb.Arn,
        Port = 80,
        DefaultActions =
        {
            new Alb.Inputs.ListenerDefaultActionArgs
            {
                Type = "forward",
