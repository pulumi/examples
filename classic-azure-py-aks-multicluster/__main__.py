...
config = Config()
password = (
    config.get_secret("password") or random.RandomPassword("pwd", length=20, special=True).result
)
ssh_public_key = config.require("sshPublicKey")

resource_group = core.ResourceGroup("aksresourcegroup")

ad_app = ad.Application("aks", display_name="aks")

ad_sp = ad.ServicePrincipal("aksSp", client_id=ad_app.client_id)
...
for config in aks_cluster_config:
    cluster = containerservice.KubernetesCluster(
        ...
        service_principal=containerservice.KubernetesClusterServicePrincipalArgs(
            client_id=ad_app.client_id, client_secret=ad_sp_password.value
        ),
