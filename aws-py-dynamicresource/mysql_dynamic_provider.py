...
class SchemaProvider(ResourceProvider):
    ...
    def create(self, args):
        ...
        return CreateResult(
            "schema-" + binascii.b2a_hex(os.urandom(16)).decode("utf-8"), outs=args
        )
    ...
    def diff(self, id, old_inputs, new_inputs):
        ...
        # recreating it
        replaces = []
        if old_inputs["server_address"] != new_inputs["server_address"]:
            replaces.append("server_address")
        if old_inputs["database_name"] != new_inputs["database_name"]:
            replaces.append("database_name")
        if old_inputs["creation_script"] != new_inputs["creation_script"]:
            replaces.append("creation_script")
