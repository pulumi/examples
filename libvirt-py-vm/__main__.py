...
libvirt_provider = libvirt.Provider(f"{basename}-libvirt", uri=libvirt_server.libvirt_remote_uri)

# Create a storage pool for the KVM VM that is going to be launched.
vm_pool = libvirt.Pool(
    f"{basename}-vm_pool",
...
test_cmd = Output.concat(
    "echo virsh list | ssh -i ",
    libvirt_server.ssh_priv_key_file,
    " ",
    libvirt_server.username,
    "@",
    libvirt_server.ip,
)
