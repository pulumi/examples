...
class ServiceDeployment(ComponentResource):
    ...
    def __init__(
        ...
        container = ContainerArgs(
            ...
            ports=[ContainerPortArgs(container_port=p) for p in ports]
            if ports
            else None,
        )
        ...
        self.service = Service(
            ...
            spec=ServiceSpecArgs(
                ports=[ServicePortArgs(port=p, target_port=p) for p in ports]
                if ports
                else None,
        ...
        if allocate_ip_address:
            if is_minikube:
                ...
                self.ip_address = ingress.apply(
                    lambda i: ingress.ip or ingress.hostname or ""
                )
