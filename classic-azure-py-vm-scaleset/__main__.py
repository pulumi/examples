...
domain = (
    config.get("domain")
    or random.RandomString("domain", length=10, number=False, special=False, upper=False).result
)
...
ssh_probe = lb.Probe("ssh-probe", loadbalancer_id=load_balancer.id, port=application_port)
