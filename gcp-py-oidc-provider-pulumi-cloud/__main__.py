...
identity_provider = iam.WorkloadIdentityPoolProvider(
    ...
    oidc=iam.WorkloadIdentityPoolProviderOidcArgs(issuer_uri=issuer, allowed_audiences=[audience]),
)
...
iam_policy_binding = serviceaccount.IAMBinding(
    ...
    members=identity_pool.name.apply(lambda name: [f"principalSet://iam.googleapis.com/{name}/*"]),
)
