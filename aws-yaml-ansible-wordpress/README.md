[![Deploy this example with Pulumi](https://www.pulumi.com/images/deploy-with-pulumi/dark.svg)](https://app.pulumi.com/new?template=https://github.com/pulumi/examples/blob/master/aws-yaml-ansible-wordpress/README.md#gh-light-mode-only)
...
1. Generate a key pair which will be used to access your EC2 instance over SSH:

    ```bash
    ssh-keygen -f wordpress-keypair
    ```

   This will create two files: your private (`wordpress-keypair`) and your public (`wordpress-keypair.pub`) keys. Keep your private key safe, since anybody with access to it can log into your EC2 machine! Note that you may choose a different file name if you're creating multiple stacks, for instance.

1. Set the required configuration variables, choosing any valid AWS region. The code is written in such a way to work in any AWS region, including fetching the right Amazon Linux 2 AMI and availability zones:

    ```bash
    pulumi config set aws:region us-east-1
    pulumi config set publicKeyPath wordpress-keypair.pub
    pulumi config set privateKeyPath wordpress-keypair
    pulumi config set dbPassword Sup45ekreT#123 --secret
    ```
    ...
    pulumi config set dbName wordpressdb
    pulumi config set dbUsername admin
    pulumi config set ec2InstanceSize t3.small
    ```

1. Run `pulumi up`. This will do all of the magic, and you'll see various things going on in the output: resources being created in AWS, commands being run locally and remotely, and the Ansible Playbook running and provisioning your Wordpress server:
...
1. After a few minutes, your new server will be ready! Its automatically-allocated EIP is printed at the end as `url`. You can access it with the `pulumi stack output` command:
...
1. Because of the network configuration, the EC2 server is available over port 80 on the Internet. (The RDS database, on the other hand, is not.) Let's `curl` the endpoint:
...
1. From there, feel free to experiment. You can simply make edits, rerun `pulumi up`, and it will incrementally update and deploy any changes you have made.

## Cleaning up

After you're done, destroy your stack and remove it, eliminating all AWS resources:
