[![Deploy this example with Pulumi](https://www.pulumi.com/images/deploy-with-pulumi/dark.svg)](https://app.pulumi.com/new?template=https://github.com/pulumi/examples/blob/master/yaml-math-and-objects/README.md#gh-light-mode-only)
[![Deploy this example with Pulumi](https://get.pulumi.com/new/button-light.svg)](https://app.pulumi.com/new?template=https://github.com/pulumi/examples/blob/master/yaml-math-and-objects/README.md#gh-dark-mode-only)

# Math, collection, and string manipulation in YAML

In addition to programming languages such as Python, Go, TypeScript, and C#, Pulumi also supports YAML.
...
3. Tinker with the stack config values in `Pulumi.yaml` or via `pulumi config set` commands to see what happens when you:
   ...
   * Disable using lowercase characters.
     ...
     * Run `pulumi up` and you should see that there are no lowercase characters and the stack output shows `minLower` set to 0.
...
* [pulumi-std Function List](https://github.com/pulumi/pulumi-std/blob/master/FUNCTION_LIST.md)
  * These are functions that are available to YAML programs.

## Tips and tricks for using the pulumi-std functions

Start with the functions list page in the `pulumi-std` repo, [pulumi-std Function List](https://github.com/pulumi/pulumi-std/blob/master/FUNCTION_LIST.md).
