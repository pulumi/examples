...
only_test:
	cd misc/test && go test ./... --timeout 4h -count=1 -short -parallel 40 --tags=all

specific_test_set:
	echo "running $(TestSet) Acceptance Tests"
	cd misc/test && go test . --timeout 4h -count=1 -short -parallel 40 --tags=all --run=TestAcc$(TestSet)
