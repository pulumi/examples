...
def s3_region_check_validator(
    stack: StackValidationArgs, report_violation: ReportViolation
):
    for resource in stack.resources:
        if resource.resource_type == "aws:s3/bucket:Bucket":
            if (
                "region" in resource.props
                and resource.props["region"] != required_region
            ):
                report_violation(
                    f"Bucket, {resource.name}, must be in region {required_region}"
                )
...
PolicyPack(
    ...
    policies=[s3_region_check, s3_count_check],
)
